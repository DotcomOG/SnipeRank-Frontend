// TEST-MARKER: snipe-server v3.1.1 — ensure /api/full exists (Render uses sniperank-v2-dev)
...
function fallbackPayload(url, reason = "fallback", snippet = "") {
  return {
    ...
    needsAttention: [
      "[PRIORITY: High] Structured Data Coverage: Add Organization, WebSite, and relevant Service/Product schemas sitewide.",
      "[PRIORITY: Medium] Meta Description Gaps: Author concise task-focused descriptions per page.",
      "[PRIORITY: Medium] Image Alt Text Coverage: Add descriptive alt attributes to key images."
    ],
    engineInsights: [
      ...
      "Gemini: Strengthen entity signals (schema + clear brand/service/location mentions).",
...
async function analyzeWebsite(url) {
  try {
    const response = await axios.get(url, { timeout: 10000, headers: { "User-Agent": "SnipeRank SEO Analyzer Bot" }});
    const $ = cheerio.load(response.data);
    const analysis = { working: [], needsAttention: [], insights: [] };

    // HTTPS
    if (url.startsWith("https://")) analysis.working.push({ title: "SSL Security Implementation", description: "Your site uses HTTPS encryption, which builds trust with AI crawlers and search algorithms." });
    else analysis.needsAttention.push({ title: "SSL Certificate Missing", description: "Your site lacks HTTPS encryption, which is now a baseline requirement for AI systems." });

    // Title
    const title = $("title").text();
    if (title && title.length > 0 && title.length <= 60) analysis.working.push({ title: "Meta Title Optimization", description: `Your page title "${title.substring(0,40)}..." is properly sized and helps AI systems understand your page focus.` });
    else if (title && title.length > 60) analysis.needsAttention.push({ title: "Meta Title Length Issues", description: "Your page titles exceed recommended character limits, reducing AI comprehension." });
    else analysis.needsAttention.push({ title: "Missing Page Titles", description: "Critical pages lack proper title tags, preventing AI systems from understanding content." });

    // Description
    const metaDesc = $('meta[name="description"]').attr("content");
    if (metaDesc && metaDesc.length > 0) analysis.working.push({ title: "Meta Description Present", description: "Your pages include meta descriptions that help AI systems understand content context." });
    else analysis.needsAttention.push({ title: "Meta Description Gaps", description: "Missing meta descriptions reduce your control over how AI systems summarize your content." });

    // H1s
    const h1Count = $("h1").length;
    if (h1Count === 1) analysis.working.push({ title: "Proper Heading Structure", description: "Your page uses a single H1 tag with clear hierarchy, helping AI systems understand content organization." });
    else if (h1Count === 0) analysis.needsAttention.push({ title: "Missing H1 Structure", description: "Pages lack proper H1 headings, making it difficult for AI systems to identify main topics." });
    else analysis.needsAttention.push({ title: "Multiple H1 Tags Detected", description: "Multiple H1 tags create content hierarchy confusion for AI parsers." });

    // Alt text
    const images = $("img");
    const imagesWithAlt = $("img[alt]");
    const altTextCoverage = images.length > 0 ? (imagesWithAlt.length / images.length) * 100 : 100;
    if (altTextCoverage >= 80) analysis.working.push({ title: "Image Optimization", description: `${Math.round(altTextCoverage)}% of your images include descriptive alt text, helping AI systems understand visual content.` });
    else analysis.needsAttention.push({ title: "Image Alt Text Gaps", description: `Only ${Math.round(altTextCoverage)}% of images have descriptive alt text, missing AI visibility opportunities.` });

    // Schema
    const hasSchema = $('script[type="application/ld+json"]').length > 0 || $("[itemscope]").length > 0;
    if (hasSchema) analysis.working.push({ title: "Structured Data Implementation", description: "Your site includes schema markup that helps AI engines understand your business type and services." });
    else analysis.needsAttention.push({ title: "Schema Markup Missing", description: "Your site lacks structured data that helps AI engines understand your business information." });
    ...
    while (analysis.working.length < 5) analysis.working.push(genericWorking[analysis.working.length % genericWorking.length]);
    ...
    while (analysis.needsAttention.length < 10) analysis.needsAttention.push(genericIssues[analysis.needsAttention.length % genericIssues.length]);
...
// Comprehensive analysis with OpenAI fallback
app.get("/api/full", async (req, res) => {
  ...
  try {
    ...
    const prompt = `You are an expert AI SEO specialist. Return ONLY valid JSON with keys: whatsWorking (10), needsAttention (25), engineInsights (5). No prose outside JSON.\n"""${content}"""`;
    ...
    let parsed; try { parsed = JSON.parse(json); } catch { return res.status(200).json(fallbackPayload(url, "parse_invalid_json", raw.slice(0,600))); }
    ...
    return res.status(200).json({
      ...
      meta: { analyzedAt: new Date().toISOString(), model: "gpt-4-turbo", url }
    });
  } catch (err) {
